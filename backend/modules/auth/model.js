...
const insert = (user) => {
	try {
		...
		if(!data) {
			data = []
			newUser = { id: 1, username: username, password: createHash(password), email }
		} else {
			data = JSON.parse(data)
			let found = data.find( user => user.username == username)
			if(!found) {
				let id = data.length ? data[data.length - 1].id + 1 : 1
				newUser = { id: id, username, password: createHash(password), email }
			} else return
		}
		data.push(newUser)
...
const login = (user) => {
	try {
		...
		if(!data) return
			...
			data = JSON.parse(data)
			let found = data.find( user => user.username == username && user.password == createHash(password))
			if(found) {
				return found
			} else return
		}
